...
def check_type(upload):
    if upload.name.endswith('.jpg') or upload.name.endswith('.png'):
        return 'img'
...
st.write("## 加载模型 :floppy_disk:")
model_list = ['Xception', 'EfficientNet-B4', 'CDC']
selected_model = st.selectbox("选择检测模型", model_list)
model_name = "cdc"
...
st.write("## 上传待检测图像/视频 :movie_camera:")
uploaded_file = st.file_uploader("选择图像或视频", type=["jpg", "png", "mp4"])
